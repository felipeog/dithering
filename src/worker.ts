...
self.addEventListener("message", (event: MessageEvent<TWorkerData>) => {
  ...
  const resizedPixels = getResizedPixels(
    bitmap,
    offscreenCanvas,
    context,
    maxLength
  ...
  const imageData = getImageData(ditheredPixels, context);
